export const vehicleData = {
  "Two Wheeler(up to 200cc)": {
    "Yamaha": ["YZF-R15", "MT-15", "FZ", "Fascino", "RayZR"],
    "Honda": ["Activa", "Dio", "Shine", "Unicorn", "X-Blade"],
    "Bajaj": ["Pulsar 150", "Pulsar 125", "Platina", "CT 100"],
    "TVS": ["Jupiter", "NTORQ", "Apache RTR 160", "Apache RTR 200", "Raider"],
    "Hero": ["Splendor", "Passion", "Glamour", "Xtreme 160R", "Destini"],
    "Suzuki": ["Access 125", "Burgman Street", "Gixxer"],
    "Other": ["Other"]
  },
  "Two Wheeler(Above 200cc)": {
    "Royal Enfield": ["Classic 350", "Meteor 350", "Interceptor 650", "Himalayan", "Continental GT 650"],
    "KTM": ["390 Duke", "250 Duke", "RC 390", "390 Adventure"],
    "Bajaj": ["Dominar 400", "Dominar 250", "Pulsar RS200", "Pulsar NS200"],
    "TVS": ["Apache RR 310"],
    "Honda": ["CB350", "CBR250R", "Africa Twin"],
    "Yamaha": ["YZF-R3", "MT-03"],
    "Kawasaki": ["Ninja 300", "Ninja 400", "Z900", "Ninja ZX-10R"],
    "Other": ["Other"]
  },
  "Three Wheeler": {
    "Bajaj": ["RE", "Maxima", "Compact"],
    "Piaggio": ["Ape City", "Ape Xtra", "Ape Auto Plus"],
    "Mahindra": ["Treo", "Alfa", "E-Alfa Mini"],
    "TVS": ["King"],
    "Atul Auto": ["Gem", "Rik"],
    "Other": ["Other"]
  },
  "Car(up to 1500cc)": {
    "Maruti Suzuki": ["Swift", "Baleno", "Dzire", "WagonR", "Alto", "Brezza", "Ertiga", "Fronx"],
    "Hyundai": ["i20", "Grand i10 Nios", "Venue", "Verna", "Aura", "Exter"],
    "Tata": ["Tiago", "Altroz", "Punch", "Nexon"],
    "Honda": ["Amaze", "City", "Elevate"],
    "Kia": ["Sonet", "Carens"],
    "Toyota": ["Glanza", "Urban Cruiser Taisor", "Rumion"],
    "Volkswagen": ["Polo", "Virtus 1.0", "Taigun 1.0"],
    "Skoda": ["Slavia 1.0", "Kushaq 1.0"],
    "Renault": ["Kwid", "Triber", "Kiger"],
    "Nissan": ["Magnite"],
    "Other": ["Other"]
  },
  "Car(above 1500cc)": {
    "Mahindra": ["Thar", "XUV700", "Scorpio-N", "Scorpio Classic", "Bolero"],
    "Toyota": ["Innova Crysta", "Innova Hycross", "Fortuner", "Hilux", "Camry"],
    "Tata": ["Harrier", "Safari", "Hexa"],
    "Hyundai": ["Creta", "Alcazar", "Tucson"],
    "Kia": ["Seltos", "Carnival"],
    "MG": ["Hector", "Astor", "Gloster"],
    "Jeep": ["Compass", "Meridian", "Wrangler"],
    "Volkswagen": ["Virtus 1.5", "Taigun 1.5", "Tiguan"],
    "Skoda": ["Slavia 1.5", "Kushaq 1.5", "Kodiaq", "Superb"],
    "Mercedes-Benz": ["C-Class", "E-Class", "GLC", "GLE"],
    "BMW": ["3 Series", "5 Series", "X1", "X3", "X5"],
    "Audi": ["A4", "A6", "Q3", "Q5", "Q7"],
    "Other": ["Other"]
  },
  "Van/Tractor/JCB": {
    "Mahindra": ["Arjun", "Yuvo", "Jivo", "Novo", "Supro", "Jeeto"],
    "John Deere": ["5E", "5M", "6M", "6R"],
    "Sonalika": ["Sikander", "Tiger", "Mahabali"],
    "New Holland": ["Excel", "Tx", "Simba"],
    "Massey Ferguson": ["Tafe", "Dynatrack"],
    "JCB": ["3DX", "4DX", "JS140", "JS205"],
    "Maruti Suzuki": ["Eeco", "Omni"],
    "Tata": ["Magic", "Winger"],
    "Force": ["Traveller", "Cruiser", "Toofan"],
    "Other": ["Other"]
  },
  "Bus": {
    "Volvo": ["9700", "9900", "7900", "B11R"],
    "Scania": ["Touring", "Citywide", "Interlink"],
    "Tata": ["Starbus", "Marcopolo", "Magna"],
    "Ashok Leyland": ["Oyster", "Viking", "Falcon"],
    "Mercedes-Benz": ["Tourismo", "Citaro"],
    "Other": ["Other"]
  },
  "Generator": {
    "Honda": ["Portable", "Inverter"],
    "Kirloskar": ["Green", "Chota Chilli"],
    "Cummins": ["Power", "QSK"],
    "Mahindra": ["Powerol"],
    "Caterpillar": ["Diesel Genset", "Gas Genset"],
    "Other": ["Other"]
  },
  "Truck": {
    "Tata": ["Prima", "Signa", "Ultra", "LPT", "Ace"],
    "Ashok Leyland": ["Boss", "Ecomet", "Captain", "Dost", "AVTR"],
    "Volvo": ["FH16", "FH", "FM", "FMX"],
    "Scania": ["R-Series", "G-Series", "P-Series", "V8"],
    "Mercedes-Benz": ["Actros", "Arocs", "Atego"],
    "BharatBenz": ["1015R", "1217C", "1923C", "2823C"],
    "Eicher": ["Pro 2000", "Pro 3000", "Pro 6000", "Pro 8000"],
    "Other": ["Other"]
  }
} as const;

export type VehicleType = keyof typeof vehicleData;
export type VehicleMake<T extends VehicleType> = keyof typeof vehicleData[T];
